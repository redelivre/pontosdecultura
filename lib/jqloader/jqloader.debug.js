(function (jQuery) {
    jQuery.fn.ajaxloader = function (msn)
    {
        ...
        jQuery('.Ajax-Loader-Image').css({'top' : ((jQuery(window).height() / 2) - (jQuery(".Ajax-Loader-Image img").width())), 'left' : ((jQuery('.Ajax-Loader').width()/2)-(jQuery("#progressbar").width())/2)});
        jQuery('.Ajax-Loader-Text').css({'margin-left' : ("-"+(jQuery('.Ajax-Loader-Text').width()/4)+"px")});
        ajaxloader.hide();
